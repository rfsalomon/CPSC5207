...
function generateOrg3Definition() {
  ...
  set -x
  configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.cpsc5207.ca/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org3.cpsc5207.ca/peers/peer0.org3.cpsc5207.ca/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3.cpsc5207.ca" ]; then
    generateOrg3
