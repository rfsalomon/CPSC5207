...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.cpsc5207.ca/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.cpsc5207.ca/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.cpsc5207.ca/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.cpsc5207.ca/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.cpsc5207.ca/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.cpsc5207.ca

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.cpsc5207.ca-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.cpsc5207.ca-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.cpsc5207.ca

    mkdir -p organizations/ordererOrganizations/cpsc5207.ca/ca
    mkdir -p organizations/ordererOrganizations/cpsc5207.ca/msp/cacerts
    mkdir -p organizations/ordererOrganizations/cpsc5207.ca/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/cpsc5207.ca/orderers
    mkdir -p organizations/ordererOrganizations/cpsc5207.ca/tlsca

    CERT_DIR=organizations/ordererOrganizations/cpsc5207.ca

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.cpsc5207.ca-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.cpsc5207.ca-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/cpsc5207.ca/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/cpsc5207.ca/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.cpsc5207.ca-cert.pem"
