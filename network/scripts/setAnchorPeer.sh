...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.org1.cpsc5207.ca"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.org2.cpsc5207.ca"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.org3.cpsc5207.ca"
    PORT=11051
  else
...
updateAnchorPeer() {
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.cpsc5207.ca -c $CHANNEL_NAME -f ${TEST_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
